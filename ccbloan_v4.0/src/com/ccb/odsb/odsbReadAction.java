...
/**
     主菜单：ODSB数据导入处理
     一、ODSB数据读取
         1、清空临时表（临时贷款申请表ln_odsb_loanapply）
         2、读ODSB贷款申请表数据，条件是：申请日期>=2010-01-01 或 开户日期>=2010-01-01，将数据装入本系统的临时贷款申请表ln_odsb_loanapply
     二、贷款信息自动补充
         0、检查
            ①根据ln_loanapply中“待补充资料标志”为“1”的记录， 与ln_odsb_loanapply中相同贷款申请号的记录比较 “姓名”“金额”“机构”，
             如有不同，弹出excel显示。
            ②查询ln_odsb_loanapply中的proj_no是否在抵押系统中存在（合作项目表），如果不存在则弹出excel。
         1、锁定系统 在批量更新前运行【0.检查】，如果有不一致地方，则返回，并提示相关信息；
         2、根据ln_loanapply中“待补充资料标志”不为“0” 的数据记录处理临时贷款申请表ln_odsb_loanapply，
           补充ln_loanapply相关信息（除了申请序号，客户经理，），并更新ln_mortinfo表（计算抵押到期日）
         3、顺序处理临时贷款申请表 追加新数据：
            insert ln_loanapply select 1，2，3 ，？ from ln_odsb_loanapply where loanid not in (select loanid from ln_loanapply)
            判断ln_odsb_loanapply开户日期为空的，ln_loanapply中的“待补充资料标志”设为“2”，非空的设为“0”
         4、拼音处理
         5、解锁
 */
    ...
    public int readFromODSB() {
        ...
        for (int i = 0; i < this.req.getRecorderCount(); i++) {
            try {
                ...
                rtn = dc.executeUpdate(" insert into ln_odsb_coopproj  select " +
                        " PROJ_NO," +
                        "ODS_SRC_DT," +
                        "CORPID," +
                        "CORPNAME," +
                        ...
                        "REMARKS," +
                        "BANKID," +
                        "null as STD_OPUN_COD," +
                        "OPERID," +
                        "INVENUM," +
                        "USETIMELIMT " +
                        " from  ODSBDATA.BF_AGT_LNP_COOP_CAPROJINFO@odsb a ");
                if (rtn < 0) {
                    this.res.setType(0);
...
//                        + " where a.APLY_DT>='2010-01-01' or a.CUST_OPEN_DT>='2010-01-01' ");
                rtn = dc.executeUpdate(" insert into ln_odsb_loanapply  select " +
                        "LOANID," +
                        "LN_ACCT_NO," +
                        "ODS_SRC_DT," +
                        "CUST_NO," +
                        ...
                        "CORPID," +
                        "CUST_NAME" +
                        " from  ODSBDATA.BF_AGT_LNP_CTRT_APPLY@odsb a ");
                if (rtn < 0) {
                    this.res.setType(0);
                    this.res.setResult(false);
                    this.res.setMessage(PropertyManager.getProperty("300"));
    ...
    public int ODSBUpdate() {
        ...
        //先检查是否已在处理中  20131219 zr
        RecordSet rs = dc.executeQuery("select syslockstatus from SYS_LOCK");
        while (rs.next()) {
            String syslock = rs.getString("syslockstatus");
            if ("1".equals(syslock)) {
                logger.error("根据ODSB更新贷款数据的处理已在进行中...");
                this.res.setType(0);
                this.res.setResult(false);
                this.res.setMessage("根据ODSB更新贷款数据的处理已在进行中...");
                return -1;
            }
        }
        ...
        logger.info("===根据ODSB更新贷款数据 begin:" + BusinessDate.getTodaytime());
        try {
            for (int i = 0; i < this.req.getRecorderCount(); i++) {
                // 加系统锁
                // 系统锁单独事务，finally解锁
                ...
                logger.info("check(“姓名”“金额”“机构) begin:" + BusinessDate.getTodaytime());
                rs = dc.executeQuery(CHECKAMT_SQL);
                while (rs.next()) {
                ...
                logger.info("check(“姓名”“金额”“机构) end:" + BusinessDate.getTodaytime() + " 耗时:" + (System.currentTimeMillis() - startTime) / 1000);
                // ②查询ln_odsb_loanapply中的proj_no是否在抵押系统中存在（合作项目表），如果不存在则弹出excel。
                startTime = System.currentTimeMillis();
                logger.info("check(合作项目表) begin:" + BusinessDate.getTodaytime());
                rs = dc.executeQuery(CHECKPROJ_SQL);
                while (rs.next()) {
                ...
                logger.info("check(合作项目表) end:" + BusinessDate.getTodaytime() + " 耗时:" + (System.currentTimeMillis() - startTime) / 1000);
                dc.setAuto(false);
                //----------------------------------------------批量更新------------------------------------------------
                startTime = System.currentTimeMillis();
                logger.info("批量更新，数据完整标志不为0 begin:" + BusinessDate.getTodaytime());
                updateCnt = dc.executeUpdate(""
                        ...
                        + " and a.NEEDADDCD <>'0'"
                        + " and b.ln_typ not in ('033','433')"
                        // zhanrui odsb数据库的 ods_src_dt字段可能不起作用，暂时屏蔽
                        //+ " and a.ods_src_dt <> b.ods_src_dt"
                        + " )"
                        + " set"
                        ...
                        + " BANKID = BANKID2,"    //重要，与下一个SQL的唯一区别之处  b.ln_typ  ('033','433')
                        + " OPERID = OPERID2,"
                ...
                logger.info("批量更新，数据完整标志不为0 end:" + BusinessDate.getTodaytime() + " 耗时:" + (System.currentTimeMillis() - startTime) / 1000);

                //--------------------------------------- 更新抵押到期日-------------------------------------------------
                startTime = System.currentTimeMillis();
                logger.info("更新抵押到期日  begin:" + BusinessDate.getTodaytime());
                rs = dc.executeQuery(""
...
*/
                ...
                logger.info("更新抵押到期日  end:" + BusinessDate.getTodaytime() + " 耗时:" + (System.currentTimeMillis() - startTime) / 1000);

                //---------------------------------------------顺序处理临时贷款申请表 追加新数据---------------------------
                // 新导入的数据NEEDADDCD 为2
                // 每条新导入的数据生成一个唯一的nbxh
                startTime = System.currentTimeMillis();
                logger.info("贷款表新增数据  begin:" + BusinessDate.getTodaytime());
                pst = null;
                String strSQL = "";
                ...
                pst.clearParameters();
                pst.clearBatch();
                logger.info("贷款表新增数据  end:" + BusinessDate.getTodaytime() + " 耗时:" + (System.currentTimeMillis() - startTime) / 1000);

                startTime = System.currentTimeMillis();
                logger.info("更新数据完整状态标志位  begin:" + BusinessDate.getTodaytime());
                //如果贷款状态为2（已销户）则数据完整状态为0（完整），否则为导入不完整
                ...
                logger.info("更新数据完整状态标志位  end:" + BusinessDate.getTodaytime() + " 耗时:" + (System.currentTimeMillis() - startTime) / 1000);
        ...
        logger.info("===根据ODSB更新贷款数据 end:" + BusinessDate.getTodaytime() + " 耗时:" + (System.currentTimeMillis() - updateStart) / 1000 + "秒");
