        ...
        for (int i = 0; i < this.req.getRecorderCount(); i++) {
            try {
                ...
                if (info.insert() < 0) {
                    ...
                    this.res.setMessage("LN_SPCLBUS_INFO添加失败，流水号重复或字段填写有误。");
                    return -1;
                }
        ...
        for (int i = 0; i < this.req.getRecorderCount(); i++) {
            try {
                ...
                if (info.updateByWhere(" where flowsn='" + req.getFieldValue(i, "flowsn") + "'") < 0) {
                    this.res.setType(0);
                    this.res.setResult(false);
                    this.res.setMessage("保存ln_spclbus_info表出错。");
                    return -1;
                }
                ...
                if (flow.getRecversion() != Integer.parseInt(req.getFieldValue(i, "af_recversion"))) {
                    this.res.setType(0);
                    this.res.setResult(false);
                    this.res.setMessage("LN_SPCLBUS_FLOW修改失败, 版本号检查错误。");
                    return -1;
                }
                String  flowstat =  req.getFieldValue(i, "flowstat");
//                if (flowstat == null) {
...
//                }
                ...
                if (flow.updateByWhere(" where pkid='" + req.getFieldValue(i, "af_pkid") + "'") < 0) {
                    this.res.setType(0);
                    this.res.setResult(false);
                    this.res.setMessage("LN_SPCLBUS_FLOW修改失败。");
                    return -1;
                }
