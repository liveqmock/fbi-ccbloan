...
public class SpclBusFlowAction extends Action {
    // 日志对象
    private static final Log logger = LogFactory.getLog(SpclBusFlowAction.class);
    // 贷款信息对象
    LNSPCLBUSFLOW flow = null;
    // 流水日志表
    LNTASKINFO task = null;

    public int add() {
        flow = new LNSPCLBUSFLOW();
        for (int i = 0; i < this.req.getRecorderCount(); i++) {
            try {
                ...
                for (String flowsn : flowsnArr) {
                    LNSPCLBUSINFO lnarchiveinfo = LNSPCLBUSINFO.findFirst(" where flowsn='" + flowsn +"'");
                    if (lnarchiveinfo == null) {
                        this.res.setType(0);
                        this.res.setResult(false);
                        this.res.setMessage("LN_SPCLBUS_FLOW添加失败, 此笔贷款资料未初始化，流水号：" + flowsn);
                        return -1;
                    }
                }
                ...
                for (String flowsn : flowsnArr) {
                    ...
                    if (flow.insert() < 0) {
                        this.res.setType(0);
                        this.res.setResult(false);
                        this.res.setMessage("LN_SPCLBUS_FLOW添加失败。");
                        return -1;
                    }
