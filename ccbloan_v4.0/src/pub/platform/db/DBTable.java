    ...
    public String writeExcel_new(WritableWorkbook book, String SqlStr, String whereStr, String fieldCN, String fieldType, String fieldWidth, String visible, String enumType) {
        ...
        try {
            ...
            for (int i = 0; i < fieldCNArr.length; i++) {
                ...
                columnIndex++;
                label = new Label(columnIndex, 1, fieldCNArr[i], wcf_title);
                // 将定义好的单元格添加到工作表中
                sheet.addCell(label);
                sheet.setColumnView(columnIndex, Integer.parseInt(fieldWidthArr[i]) * 2);
            }
...
//            WritableCellFormat wcf_number = new WritableCellFormat(nf);
            ...
            while (rs.next()) {
                ...
                for (int i = 0; i < columnCount; i++) {
                    ...
                    columnIndex++;
                    if (fieldTypeArr[i].toLowerCase().equals("number")) {
                        numberCell = new jxl.write.Number(columnIndex, reindex + 1, rs.getDouble(i), wcf_number);
                        sheet.addCell(numberCell);
                    } else if (fieldTypeArr[i].toLowerCase().equals("money")) {
                        numberCell = new jxl.write.Number(columnIndex, reindex + 1, rs.getDouble(i), wcf_money);
                        sheet.addCell(numberCell);
                    } else if (fieldTypeArr[i].toLowerCase().equals("center")) {
                        label = new Label(columnIndex, reindex + 1, rs.getString(i), wcf_text_center);
                        sheet.addCell(label);
                        ...
                        label = new Label(columnIndex, reindex + 1, enumStr, wcf_text_center);
                        sheet.addCell(label);
                    } else {
                        label = new Label(columnIndex, reindex + 1, rs.getString(i), wcf_text_left);
                        sheet.addCell(label);
                    }
                }
