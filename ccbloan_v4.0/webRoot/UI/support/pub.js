...
var MSG_BATCH_INFO_SIGN = "确定批量资料签收该批数据吗？";
var MSG_BATCH_EXPRESS_RTN = "确定批量快递回证该批数据吗？";
var MSG_BATCH_PAPER_SIGN = "确定批量权证签收该批数据吗？";
...
/** 170营销信息录入 */
var BUSINODE_170 = "170"
/** 180营销信息审核 */
var BUSINODE_180 = "180"
/** 190个贷信用评定 */
var BUSINODE_190 = "190"
/** ----------------------------------------------------------------------------------------------------------------- */
...
function showDialog(width, height, url, arg, resizable) {
    ...
    var time = getDateString(date) + "_" + getTimeString(date);
    if (url.indexOf("?") > 0) {
...
function resetDialogHeight() {
    ...
    if (ua.lastIndexOf("MSIE 6.0") != -1) {
        if (ua.lastIndexOf("Windows NT 5.1") != -1) {
            //alert("xp.ie6.0");
            var height = window.dialogHeight = (height + 102) + "px";
