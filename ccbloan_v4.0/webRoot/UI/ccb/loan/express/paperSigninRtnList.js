...
function cbRetrieve_Click(formname) {
    ...
    if (trimStr(document.getElementById("STARTDATE").value) != "") {
        whereStr += " and b.EXPRESSRTNDATE >='" + trimStr(document.getElementById("STARTDATE").value) + "' ";
    }
    if (trimStr(document.getElementById("ENDDATE").value) != "") {
        whereStr += " and b.EXPRESSRTNDATE <='" + trimStr(document.getElementById("ENDDATE").value) + "' ";
    }
...
function loanRegistedTab_editRecord_click() {
    ...
    // 抵押编号
    arg.mortID = mortID;
    var ret = dialog("paperSigninRtnEdit.jsp?mortID=" + mortID + "&loanID=" + loanID +  "&recVersion=" + recVersion + "&doType=edit", arg, sfeature);
...
function loanRegistedTab_batchEdit_click() {
    ...
    for ( var i = 0; i < tab.rows.length; i++) {
        if (tab.rows[i].cells[0].children[0] != undefined && tab.rows[i].cells[0].children[0].checked) {
            ...
            // 快递回证日期：当前日期
            _cell = document.createElement("td");
            _cell.setAttribute("fieldname", "EXP_PAPER_SIGNIN_DATE");
            _cell.style.display = "none";
            _cell.setAttribute("fieldtype", "text");
            var date = new Date();
    ...
    if (confirm(MSG_BATCH_PAPER_SIGN)) {
        for ( var i = 0; i < tab.rows.length; i++) {
            if (tab.rows[i].cells[0].children[0] != undefined && tab.rows[i].cells[0].children[0].checked) {
                // ---
                ...
                //权证签收
                _cell.setAttribute("oldvalue", "21A");
                tab.rows[i].appendChild(_cell);
...
function loanRegistedTab_expExcel_click() {
    ...
    document.getElementById("queryForm").action = "dataRegRpt.jsp";
