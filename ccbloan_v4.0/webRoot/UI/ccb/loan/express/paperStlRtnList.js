...
function loanRegistedTab_editRecord_click() {
    ...
    // 抵押编号
    arg.mortID = mortID;
    var ret = dialog("paperStlRtnEdit.jsp?mortID=" + mortID + "&loanID=" + loanID +  "&recVersion=" + recVersion + "&doType=edit", arg, sfeature);
...
function loanRegistedTab_batchEdit_click() {
    ...
    if (confirm(MSG_BATCH_EXPRESS_RTN)) {
        for ( var i = 0; i < tab.rows.length; i++) {
            if (tab.rows[i].cells[0].children[0] != undefined && tab.rows[i].cells[0].children[0].checked) {
                // ---
...
function loanRegistedTab_expExcel_click() {
    ...
    document.getElementById("queryForm").action = "/UI/ccb/loan/express/dataRegRpt.jsp";
