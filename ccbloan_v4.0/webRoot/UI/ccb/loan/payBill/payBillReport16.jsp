...
<%
    Log logger = LogFactory.getLog("payBillReport16.jsp");
    ...
    try {
        do {
            ...
            response.addHeader("Content-Disposition", "attachment; filename=" + "payBill16_" + new SimpleDateFormat("yyyyMMdd").format(new Date()) + ".xls");
            // ----------------------------根据模板创建输出流----------------------------------------------------------------
            //得到报表模板
            String rptModelPath = PropertyManager.getProperty("REPORT_ROOTPATH");
            String rptName = rptModelPath + "payBill16.xls";
            ...
            */
            ...
            ws.addCell(lbl);
            Number nLbl = new Number(beginCol + 7, i * step + beginRow, allOrgPerformPaymPay.doubleValue(), wcf_bold_right);
            ws.addCell(nLbl);
