...
//            String loantype2 = "t.ln_typ  in ('033','433')";
            ...
            // 统计查询语句
            String sql = ""
                    + " select bankid,bankname,custmgr_name,amt1,amt2,round(rate1/amt1,2) as rate1,round(rate2/amt2,2) as rate2 , " +
...
//      WritableCellFormat wcf_right = new WritableCellFormat(new WritableFont(WritableFont.COURIER,11,WritableFont.NO_BOLD,false));
            ...
            while (rs.next()) {
                ...
                ws.addCell(lbl);
                // 客户经理名称合并
                ws.mergeCells(beginCol + 1, i + beginRow + cnt, beginCol + 1, i + beginRow + 1 + cnt);

                // 贷款种类
                lbl = new Label(beginCol + 2, i + beginRow + 1 + cnt, RPTLABEL04, wcf_center);
                ws.addCell(lbl);
                // 贷款浮动比例
            ...
            */
            ...
            sql = "select sum(amt1) as amt1 ,sum(amt2) as amt2, round(sum(totalrate1) / sum(amt1), 2) as rate1, round(sum(totalrate2) / sum(amt2), 2) as rate2  from (" + sql + ")";
            ...
            while (rs.next()) {
                ...
                ws.addCell(lbl);
                // 合计
                lbl = new Label(beginCol+1, i + beginRow + cnt, "合计", wcf_bold_center);
                ws.addCell(lbl);
                // 贷款种类
                ...
                ws.mergeCells(beginCol + 1, i + beginRow + cnt, beginCol + 1, i + beginRow + 1 + cnt);
                // 贷款种类
                lbl = new Label(beginCol + 2, i + beginRow + 1 + cnt, RPTLABEL04, wcf_bold_center);
                ws.addCell(lbl);
                // 贷款浮动比例
