...
//采用do while结构便于在主流程中监测错误发生后退出主程序；
    try {
        do {
            ...
            response.addHeader("Content-Disposition", "attachment; filename=" + CcbLoanConst.NORTN_RPT_MODEL_01 + ".xls");
            // ----------------------------根据模板创建输出流----------------------------------------------------------------
            //得到报表模板
            String rptModelPath = PropertyManager.getProperty("REPORT_ROOTPATH");
            String rptName = rptModelPath + CcbLoanConst.NORTN_RPT_MODEL_01 + ".xls";
            ...
            if (startdateflag == 1 && enddateflag == 0) {
                beans.put("condidate", " 考核截至日期:" + enddate);
            } else if (startdateflag == 0 && enddateflag == 1) {
                beans.put("condidate", " 考核起始日期:" + startdate);
            } else if (startdateflag == 0 && enddateflag == 0) {
                beans.put("condidate", " 考核日期:" + startdate + "至" + enddate);
            } else {
                beans.put("condidate", " ");
            }
        ...
        logger.error("生成签约放款回证考核表一时出现错误。");
        e.printStackTrace();
    } finally {
        //todo: 链接
