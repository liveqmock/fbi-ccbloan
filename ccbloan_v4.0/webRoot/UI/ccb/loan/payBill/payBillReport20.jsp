...
<%
    Log logger = LogFactory.getLog("payBillReport20.jsp");
    ...
    //每万元买单价格基准
    //旧数据小机构买单 全流程中心 371997709 ， 371997909 ， 371998009 (胶州，平度，莱西)
    ...
    try {
        do {
            ...
            response.addHeader("Content-Disposition", "attachment; filename=" + "payBill20_" + new SimpleDateFormat("yyyyMMdd").format(new Date()) + ".xls");
            // ----------------------------根据模板创建输出流----------------------------------------------------------------
            //得到报表模板
            String rptModelPath = PropertyManager.getProperty("REPORT_ROOTPATH");
            String rptName = rptModelPath + "payBill20.xls";
            ...
            rs = conn.executeQuery("select enuitemexpand as name,to_number(enuitemvalue) as value from ptenudetail where enutype in('PAYBILLRPTPARAM') ");
            while (rs.next()) {
            ...
            String sql = ""
                    ...
                    + "        null"
                    + "     end) as rate3  "
                    + " from ln_loanapply t join ptdept on t.bankid=ptdept.deptid " +
                    whereStr
                    ...
                    "   and t.bankid in (select deptid from ptdept where fillstr10='4')" +
                    "   and t.loanid not in (select loanid  from ln_promotioncustomers pcms where pcms.status='4')" +
            ...
            String sql1 = ""
                    ...
                    + " from ln_loanapply t"
                    + whereStr
                    + " and t.bankid in(select deptid from ptdept start with deptid='371980000' connect by prior deptid=parentdeptid) "
                    + " group by bankid"
                    + " order by bankid"
                    + " ) ";
