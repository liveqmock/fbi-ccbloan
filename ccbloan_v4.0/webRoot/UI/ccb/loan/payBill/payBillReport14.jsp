<!--
/*********************************************************************
* 功能描述: 买单工资查询统计14 (表五) (类似表三 但统计项目不同）
* 作 者:
* 开发日期: 2010/12/10
* 修 改 人:
* 修改日期:
...
<%
    ...
    try {
        do {
            ...
            response.addHeader("Content-Disposition", "attachment; filename=" + "payBill14.xls");
            // ----------------------------根据模板创建输出流----------------------------------------------------------------
            //得到报表模板
            String rptModelPath = PropertyManager.getProperty("REPORT_ROOTPATH");
            String rptName = rptModelPath + "payBill14.xls";
            ...
            // 统计查询语句
            String sql = ""
                    + " select bankid,bankname,amt1,amt2,amt3,round(rate1/amt1,2) as rate1,round(rate2/amt2,2) as rate2,round(rate3/amt3,2) as rate3 , " +
            ...
            while (rs.next()) {
                ...
                ws.addCell(lbl);
                // 贷款种类
                lbl = new Label(beginCol + 1, i + beginRow + 1 + cnt, RPTLABEL02, wcf_center);
                ws.addCell(lbl);
                // 贷款浮动比例
                ...
                // 贷款种类
                lbl = new Label(beginCol + 1, i + beginRow + 2 + cnt, RPTLABEL03, wcf_center);
                ws.addCell(lbl);
                // 贷款浮动比例
                nLbl = new Number(beginCol + 2, i + beginRow + 2 + cnt, rs.getDouble("rate3"), wcf_right);
                ws.addCell(nLbl);
                // 贷款发放额
                nLbl = new Number(beginCol + 3, i + beginRow + 2 + cnt, rs.getDouble("amt3"), wcf_right);
                ws.addCell(nLbl);
                // 计算总额
            ...
            */
            ...
            while (rs.next()) {
                ...
                ws.addCell(lbl);
                // 贷款种类
                lbl = new Label(beginCol + 1, i + beginRow + cnt, RPTLABEL01, wcf_bold_center);
                ws.addCell(lbl);
                // 贷款浮动比例
                Number nLbl = new Number(beginCol + 2, i + beginRow + cnt, rs.getDouble("rate1"), wcf_bold_right);
                //lbl = new Label(beginCol + 2, i + beginRow + cnt, "平均" + (totalRate_1 / rowNum) , wcf_bold_right);
                ws.addCell(nLbl);
                // 贷款发放额
                nLbl = new Number(beginCol + 3, i + beginRow + cnt, rs.getDouble("amt1"), wcf_bold_right);
                ws.addCell(nLbl);
                //---------------------------第二行-------------------------------
                ws.setRowView(i + beginRow + 1 + cnt, rowHeight, false);
                // 机构名称
                ...
                ws.addCell(lbl);
                // 贷款种类
                lbl = new Label(beginCol + 1, i + beginRow + 1 + cnt, RPTLABEL02, wcf_bold_center);
                ws.addCell(lbl);
                // 贷款浮动比例
                nLbl = new Number(beginCol + 2, i + beginRow + 1 + cnt, rs.getDouble("rate2"), wcf_bold_right);
                ws.addCell(nLbl);
                // 贷款发放额
                nLbl = new Number(beginCol + 3, i + beginRow + 1 + cnt, rs.getDouble("amt2"), wcf_bold_right);
                ws.addCell(nLbl);
                //---------------------------第三行-------------------------------
                ws.setRowView(i + beginRow + 2 + cnt, rowHeight, false);
                // 机构名称
                ...
                ws.mergeCells(beginCol, i + beginRow + cnt, beginCol, i + beginRow + 2 + cnt);
                // 贷款种类
                lbl = new Label(beginCol + 1, i + beginRow + 2 + cnt, RPTLABEL03, wcf_bold_center);
                ws.addCell(lbl);
                // 贷款浮动比例
                nLbl = new Number(beginCol + 2, i + beginRow + 2 + cnt, rs.getDouble("rate3"), wcf_bold_right);
                ws.addCell(nLbl);
                // 贷款发放额
                nLbl = new Number(beginCol + 3, i + beginRow + 2 + cnt, rs.getDouble("amt3"), wcf_bold_right);
                ws.addCell(nLbl);
                // 行计数器加1
                i++;
            }
