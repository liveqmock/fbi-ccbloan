...
//采用do while结构便于在主流程中监测错误发生后退出主程序；
    try {
        do {
            ...
            response.addHeader("Content-Disposition", "attachment; filename=" + CcbLoanConst.NORTN_RPT_MODEL_03 + ".xls");
            // ----------------------------根据模板创建输出流----------------------------------------------------------------
            //得到报表模板
            String rptModelPath = PropertyManager.getProperty("REPORT_ROOTPATH");
            String rptName = rptModelPath + CcbLoanConst.NORTN_RPT_MODEL_03 + ".xls";
        ...
        logger.error("生成签约放款未回证考核表二时出现错误。");
        e.printStackTrace();
    } finally {
        //todo: 链接
