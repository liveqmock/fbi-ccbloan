...
function loanRegistedTab_BatchEditRecord_click() {
    ...
    for (var i = 0; i < tab.rows.length; i++) {
        if (tab.rows[i].cells[0].children[0] != undefined && tab.rows[i].cells[0].children[0].checked) {
            ...
            _cell.setAttribute("oldvalue", tab.rows[i].ValueStr.split(";")[12]);
            ...
            _cell.style.display = "none";
            _cell.setAttribute("fieldtype", "text");
            _cell.setAttribute("oldvalue", BUSINODE_040);
            tab.rows[i].appendChild(_cell);
    ...
    var arg = new Object();
    var rtn = dialog("otherBankBranchCanMortBatchEdit.jsp", arg, sfeature);
    var rptmortdt = "";
    var sendflag = "";
    if (rtn != undefined) {
        rptmortdt = rtn.rptmortdt;
    ...
    if (rtn != undefined) {
        for (var i = 0; i < tab.rows.length; i++) {
            if (tab.rows[i].cells[0].children[0] != undefined && tab.rows[i].cells[0].children[0].checked) {
                var _cell = document.createElement("td");
                _cell.setAttribute("fieldname", "RPTMORTDATE");
                _cell.style.display = "none";
                _cell.setAttribute("fieldtype", "text");
                _cell.setAttribute("oldvalue", rptmortdt);
                tab.rows[i].appendChild(_cell);
                var _cell = document.createElement("td");
...
function loanRegistedTab_editRecord_click() {
    ...
    // 抵押编号
    arg.mortID = mortID;
    var ret = dialog("otherBankBranchCanMortEdit.jsp?mortID=" + mortID + "&doType=apply", arg, sfeature);
 ...
 * @param mortid:抵押编号
 */
function loanRegistedTab_editSendFlag_click() {
    //增加系统锁检查
    if (getSysLockStatus() == "1") {
        alert(MSG_SYSLOCK);
        return;
    ...
    // 抵押编号
    arg.mortID = mortID;
    var ret = dialog("otherBankBranchCanMortEdit.jsp?mortID=" + mortID + "&doType=modify", arg, sfeature);
 ...
 * @param mortid:抵押编号
 */
function loanRegistedTab_BatchEditSendFlag_click() {
    // 选取数据标志
    var checked = false;
    var tab = document.all.loanRegistedTab;
    for (var i = 0; i < tab.rows.length; i++) {
    ...
    // 操作类型：edit
    arg.doType = "modify";
    var rtn = dialog("otherBankBranchCanMortBatchEdit.jsp?doType=modify", arg, sfeature);
    var rptmortdt = "";
    var sendflag = "";
    if (rtn != undefined) {
        rptmortdt = rtn.rptmortdt;
