...
function cbRetrieve_Click(formname) {
    ...
    if (trimStr(document.getElementById("custbankid").value) != "") {
        whereStr += " and bankid in (select deptid from ptdept start with deptid='" + trimStr(document.getElementById("custbankid").value) + "' connect by prior deptid=parentdeptid)";
    }
    if (trimStr(document.getElementById("RELEASECHECK").value) != "") {
        whereStr += " and status ='" + trimStr(document.getElementById("RELEASECHECK").value) + "'";
    }
...
function promotionTab_editRecord_click() {
    ...
    var arg = new Object();
    arg.doType = "edit";
    var rtn = dialog("custMgrEdit.jsp?promcustno=" + promcustno + "&doType=edit", arg, sfeature);
    if (rtn == "1") {
