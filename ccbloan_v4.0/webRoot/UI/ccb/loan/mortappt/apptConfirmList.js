 ...
 * 批量确认
 */
function mainTab_batchSignin_click() {
    // 选取数据标志
    var checked = false;
    var tab = document.all.mainTab;
    var clientNames = "";
    ...
    var rtn = dialog("apptConfirmYesEdit.jsp?doType=edit", arg, sfeature);
    ...
    if (rtn != undefined) {
        for (var i = 0; i < tab.rows.length; i++) {
            if (tab.rows[i].cells[0].children[0] != undefined && tab.rows[i].cells[0].children[0].checked) {
                ...
                appendOneTdIntoRow("appt_sendback_remark", rtn.appt_sendback_remark);
                //appendOneTdIntoRow("appt_remark", rtn.appt_remark);
                appendOneTdIntoRow("apptstatus", "20"); //确认通过
                appendOneTdIntoRow("appt_confirm_result", "10"); //确认通过
                appendOneTdIntoRow("appt_valid_flag", "1"); //有效记录
                appendOneTdIntoRow("appt_over_flag", "0"); //处理未完成
                appendOneTdIntoRow("action_type", "signin"); //申请确认
            } else {
                tab.rows[i].operate = "";
            }
        }
 ...
 * 批量预约退回确认
 */
function mainTab_batchSendback_click() {
    // 选取数据标志
    var checked = false;
    var tab = document.all.mainTab;
    var clientNames = "";
    ...
    arg.doType = "edit";
    arg.clientNames = clientNames;
    var rtn = dialog("apptConfirmNoEdit.jsp?doType=edit", arg, sfeature);
    ...
    if (rtn != undefined) {
        for (var i = 0; i < tab.rows.length; i++) {
            if (tab.rows[i].cells[0].children[0] != undefined && tab.rows[i].cells[0].children[0].checked) {
                ...
                var _cell;
                appendOneTdIntoRow("recversion", tab.rows[i].ValueStr.split(";")[11]);
                appendOneTdIntoRow("appt_sendback_reason", rtn.appt_sendback_reason);
                appendOneTdIntoRow("appt_sendback_remark", rtn.appt_sendback_remark);
                appendOneTdIntoRow("apptstatus", "02"); //退回
                appendOneTdIntoRow("appt_confirm_result", "20"); //确认退回
                appendOneTdIntoRow("appt_valid_flag", "1"); //有效记录（只有取消预约，才是无效记录）
                appendOneTdIntoRow("appt_over_flag", "1"); //处理完成（退回后（或权证回证），本次预约行为即为完成）
                appendOneTdIntoRow("action_type", "seedback"); //退回
            } else {
                tab.rows[i].operate = "";
            }
        }
