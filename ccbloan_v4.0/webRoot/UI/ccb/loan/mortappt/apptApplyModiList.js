...
function mainTab_batchEdit_click() {
    ...
    var rtn = dialog("apptApplyModiEdit.jsp?doType=edit", arg, sfeature);
    ...
    if (rtn != undefined) {
        for (var i = 0; i < tab.rows.length; i++) {
            if (tab.rows[i].cells[0].children[0] != undefined && tab.rows[i].cells[0].children[0].checked) {
                ...
                appendOneTdIntoRow("apptstatus", "01"); //待预约申请
                appendOneTdIntoRow("appt_valid_flag", "0"); //无效记录
                appendOneTdIntoRow("appt_over_flag", "0"); //处理未完成
                appendOneTdIntoRow("action_type", "apply_cancel"); //预约申请取消
            } else {
                tab.rows[i].operate = "";
            }
        }
